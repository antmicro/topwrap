...
This example has an RISC-V processor, memory, and a UART connected by Wishbone interconnect.
Interconnect is not present as a Verilog file, but is generated by Topwrap.
Unlike other ir-examples it is localized in `examples/soc`.
It has both IR modules defined in python files and System Verilog files that are parsed by `topwrap parse`.
It can be run in a simulator such as Verilator.
To run this example, the RISC-V 64-bit toolchain and Verilator simulator need to be installed.
...
This example has connected manager and two UART subordinates to AXI interconect. Interconnect is generated by Topwrap.
