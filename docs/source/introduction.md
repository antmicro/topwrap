...
As a tool, Topwrap makes it [straightforward to build](getting_started.md) complex and [synthesizable designs](#examples) by generating a design file in formats such as SystemVerilog or [IP-XACT 2022](getting_started.md#generating-ipxact-2022-files). The combination of [GUI and CLI-based](getting_started.md#generating-verilog-in-the-gui) configuration options provides for fine-tuning possibilities. Packaging multiple files is accomplished by including them in a [custom user repository](user_repositories.md), and an internal API enables repository creation using Python.
