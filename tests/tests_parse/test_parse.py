...
class TestVerilogParse:
    ...
    def test_axi_axil_adapter_ports(self, axi_axil_adapter_ports_by_dir):
        ...
        assert {
            "name": "s_axi_awid",
            "bounds": ("(AXI_ID_WIDTH-1)", "0"),
        } in axi_axil_adapter_ports_by_dir["in"]
        assert {"name": "s_axi_arlen", "bounds": ("7", "0")} in axi_axil_adapter_ports_by_dir["in"]

        assert len(axi_axil_adapter_ports_by_dir["out"]) == 22
        assert {"name": "s_axi_awready", "bounds": ("0", "0")} in axi_axil_adapter_ports_by_dir[
            "out"
        ]
        assert {
            "name": "s_axi_rid",
            "bounds": ("(AXI_ID_WIDTH-1)", "0"),
        } in axi_axil_adapter_ports_by_dir["out"]
        assert {"name": "m_axil_arprot", "bounds": ("2", "0")} in axi_axil_adapter_ports_by_dir[
            "out"
        ]
...
class TestVHDLParse:
    ...
    def test_axi_dispctrl_ports(self, axi_dispctrl_ports_by_dir):
        ...
        assert {
            "name": "S_AXIS_TDATA",
            "bounds": ("(C_S_AXIS_TDATA_WIDTH-1)", "0"),
        } in axi_dispctrl_ports_by_dir["in"]
        ...
        assert {
            "name": "DATA_O",
            "bounds": ("(C_S_AXIS_TDATA_WIDTH-1)", "0"),
        } in axi_dispctrl_ports_by_dir["out"]
