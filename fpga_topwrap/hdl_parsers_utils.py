...
def _eval_param(val, params: dict):
    ...
    if isinstance(val, str):
        ...
        if val['fn'] == 'CONCAT':
            ...
            bin_l = bin(int(bit_vector_l['value'], 16))[2:].zfill(
                bit_vector_l['width'])
            bin_r = bin(int(bit_vector_r['value'], 16))[2:].zfill(
                bit_vector_r['width'])
            return {
            ...
            bin_val = bin(int(bit_vector['value'], 16))[2:].zfill(
                bit_vector['width'])
            return {
                'value': hex(int(repeat * bin_val, 2)),
                'width': repeat * bit_vector['width']
            }
        ...
        if val['fn'] in bin_ops.keys():
            ...
            for name in params_cp.keys():
                if isinstance(params_cp[name], str):
                    params_cp[name] = int(ex.evaluate(
                        params_cp[name], params_cp).take(0))

            return _eval_param(val, params_cp)

        elif val['fn'] == 'PARAMETRIZATION':
            if val['ops'][0] == 'reg' and \
